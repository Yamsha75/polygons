...
function assertArgumentIsElement(variable, argumentNumber)
    ...
    if argumentNumber then
        error(string.format("expected element at argument %d, got %s", argumentNumber,
            variableType), 2)
    else
...
function assertArgumentType(variable, expectedType, argumentNumber)
    ...
    if argumentNumber then
        error(string.format("expected %s at argument %d, got %s", expectedType,
            argumentNumber, variableType), 2)
    else
        error(string.format("expected %s as argument, got %s", expectedType,
            variableType), 2)
    end
end
...
function assertSourceType(variable, expectedType)
    ...
    error(string.format("expected %s as event source, got %s", expectedType,
        variableType), 2)
end
